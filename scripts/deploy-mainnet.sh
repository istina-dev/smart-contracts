...
# ============================================
# Production deployment workflow for mainnet.
# ============================================
...
echo ""
echo "=================================================="
echo "MAINNET DEPLOYMENT - PRODUCTION ENVIRONMENT"
echo "=================================================="
echo ""
...
if [ "$ADMIN_SQUADS_PUBLIC_KEY" = "$FEE_PAYER_PUBLIC_KEY" ]; then
    ...
    echo "OK: Admin is not the fee-payer. Confirm manually it is Squads Vault 0."
fi
...
if [ "$AUDITED" != "yes" ]; then
    echo ""
    echo "WARNING: Audit status not confirmed for this mainnet deployment."
    echo "   Recommended: complete an independent security review before deployment."
    echo ""
    read -p "Proceed without audit confirmation? (y/n) " -n 1 -r
    echo
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        exit 1
    fi
...
echo "OK: Configured Solana CLI for mainnet"
...
if [ -n "$CURRENT_UPGRADE_AUTHORITY" ]; then
    echo "OK: Existing mainnet program detected"
    ...
    echo "OK: No readable existing mainnet upgrade authority found; treating this as a first deploy"
fi
...
if ! solana program deploy "${DEPLOY_ARGS[@]}"; then
    echo ""
    echo "ERROR: Mainnet deploy failed while uploading program data."
...
echo ""
echo "INFO: Publishing IDL (best effort)..."
...
if anchor idl "$IDL_COMMAND" "$SOLANA_PROGRAM_ID" \
  ...
  --provider.wallet "$KEYPAIR_FILE"; then
    echo "OK: IDL $IDL_COMMAND complete"
    ...
    if [ "$CURRENT_IDL_AUTHORITY" = "$ADMIN_SQUADS_PUBLIC_KEY" ]; then
      echo "OK: IDL authority already set to Squads Vault 0"
    elif [ "$CURRENT_IDL_AUTHORITY" = "$FEE_PAYER_PUBLIC_KEY" ]; then
      echo "INFO: Transferring IDL authority to Squads Vault 0..."
      if anchor idl set-authority \
        --program-id "$SOLANA_PROGRAM_ID" \
        --new-authority "$ADMIN_SQUADS_PUBLIC_KEY" \
        --provider.cluster "$ANCHOR_CLUSTER" \
        --provider.wallet "$KEYPAIR_FILE"; then
          echo "OK: IDL authority transferred to Squads Vault 0"
      else
          echo "WARNING: IDL authority transfer failed (non-fatal)."
      fi
    elif [ -n "$CURRENT_IDL_AUTHORITY" ]; then
      echo "WARNING: IDL authority is neither fee payer nor Squads Vault 0: $CURRENT_IDL_AUTHORITY"
      echo "   Review manually before future IDL upgrades."
    else
      echo "WARNING: Could not read IDL authority (non-fatal)."
    fi
else
    echo ""
    echo "WARNING: IDL publish failed (non-fatal)."
...
echo ""
echo "INFO: Skipping automatic upgrade-authority transfer in this step (safe default)."
