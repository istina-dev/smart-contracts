...
echo "✓ Configured Solana CLI for devnet"
...
if [ "$BALANCE" -lt 2000000000 ]; then
    echo ""
    echo "⚠️  Warning: Balance is low (< 2 SOL)"
...
echo ""
echo "🔎 Checking deploy balance (devnet)..."
...
if [ -n "$CURRENT_UPGRADE_AUTHORITY" ]; then
    echo "✓ Existing devnet program detected"
    ...
    echo "✓ No readable existing devnet upgrade authority found; treating this as a first deploy"
fi
...
if ! solana program deploy "${DEPLOY_ARGS[@]}"; then
    echo ""
    echo "❌ Devnet deploy failed while uploading program data."
...
echo ""
echo "ℹ️  Note: Solscan may still show Security.txt = FALSE on devnet until it re-indexes."
