...
async function main() {
  ...
  // Load admin/fee-payer keypair (path-based only).
  const keypairPath = process.env.FEE_PAYER_KEYPAIR_PATH;
  if (!keypairPath) {
    console.error(`Fee payer keypair not configured in ${envFile}`);
    console.error("Set FEE_PAYER_KEYPAIR_PATH in your environment file");
    process.exit(1);
  }
  const resolvedKeypairPath = path.resolve(keypairPath);
  if (!fs.existsSync(resolvedKeypairPath)) {
    console.error(`Fee payer keypair file not found: ${resolvedKeypairPath}`);
    process.exit(1);
  }
  const keypairData = JSON.parse(fs.readFileSync(resolvedKeypairPath, "utf8"));
  ...
  try {
    const existingConfig = await (program.account as any).globalConfig.fetch(
      globalConfig
    );
