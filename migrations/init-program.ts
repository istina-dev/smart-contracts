...
async function main() {
  ...
  // Load admin keypair (fee payer initially, path-based only)
  ...
  if (!fs.existsSync(resolvedFeePayerKeypairPath)) {
    console.error(
      `Fee payer keypair file not found: ${resolvedFeePayerKeypairPath}`
    );
    process.exit(1);
  }
  ...
  try {
    ...
    console.log("");
    console.log(
      "No action needed. Use update-admin-to-squads.ts to change admin."
    );
    return;
  } catch (e) {
  ...
  console.log(
    "1. Run update-admin-to-squads.ts to switch admin to Squads Vault 0"
  );
