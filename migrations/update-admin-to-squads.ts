...
async function main() {
  ...
  // Load current admin keypair (fee payer)
  ...
  let currentAdminKeypair: anchor.web3.Keypair;
  if (feePayerKeypairPath && fs.existsSync(feePayerKeypairPath)) {
    ...
    currentAdminKeypair = anchor.web3.Keypair.fromSecretKey(Uint8Array.from(keypairData));
  ...
  try {
    ...
    const updatedConfig = await fetchGlobalConfigWithRetry(program, globalConfig);
